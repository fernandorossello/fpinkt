...
//tag::init[]
fun <A, B> Option<A>.map2(f: (A) -> B): Option<B> = when(this) {
    is None -> None
    is Some -> Some(f(this.get))
}


fun <A, B> Option<A>.flatMap3(f: (A) -> Option<B>): Option<B> =
    when(this) {
        is None -> None
        is Some -> f(this.get)
    }
...
fun <A, B> Option<A>.flatMap_2(
    f: (A) -> Option<B>
): Option<B> = this.map(f).getOrElse { None }

fun <A> Option<A>.orElse_2(
    ob: () -> Option<A>
): Option<A> = this.map {Some(it)}.getOrElse { ob() }

fun <A> Option<A>.filter_2(
    f: (A) -> Boolean
): Option<A> = this.flatMap_2 { if(f(it)) Some(it) else None }
...
class Exercise1 : WordSpec({
    ...
    "option map" should {
        "transform an option of some value" {
            some.map2 { it * 2 } shouldBe Some(20)
        }
        "pass over an option of none" {
            none.map2 { it * 10 } shouldBe None
        }
    }
