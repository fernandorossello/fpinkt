...
import chapter5.solutions.ex13.zipWith
...
fun <A, B> Stream<A>.map(f: (A) -> B): Stream<B> =
    unfold(this) { sa: Stream<A> ->
        when(sa) {
        is Cons -> Some(f(sa.head()) to sa.tail() )
        is Empty -> None
    } }
...
fun <A> Stream<A>.take(n: Int): Stream<A> =
    unfold(this) {sa: Stream<A> ->
        when(sa) {
            ...
            is Cons ->
                if(n > 0) {
                    Some(sa.head() to sa.tail().take(n - 1))
                } else {
                    None
                }
        }
...
fun <A> Stream<A>.takeWhile(p: (A) -> Boolean): Stream<A> =
    unfold(this) {sa: Stream<A> ->
        when(sa) {
            ...
            is Cons ->
                if(p(sa.head()))
                    Some(sa.head() to sa.tail().takeWhile(p))
                else None
        }
    }
...
fun <A, B, C> Stream<A>.zipWith(
    that: Stream<B>,
    f: (A, B) -> C
): Stream<C> = unfold(this to that) { (ths: Stream<A>, tht: Stream<B>) ->
    when(ths) {
        is Empty -> None
        is Cons -> when(tht) {
            is Empty -> None
            is Cons -> Some(Pair(f(ths.head(),tht.head()),
...
fun <A, B> Stream<A>.zipAll(
    that: Stream<B>
): Stream<Pair<Option<A>, Option<B>>> = SOLUTION_HERE()
//end::init[]
